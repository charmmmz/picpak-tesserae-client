...
//     device_id, device_token and home_pubkey. A missing install_id strands
//     pairing entirely.
...
/* Build the POST .../status body. Pure so the button contract is host-testable.
 * button + button_event_id are emitted TOGETHER or not at all (a name with no id
 * is the one malformed pair the server drops); optional ip/rssi/battery are
 * omitted when unset. False (and out[0]='\0') if the body would not fit. */
