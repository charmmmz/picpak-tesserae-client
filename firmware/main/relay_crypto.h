...
// the Tesserae repo -- authoritative for all three sides (home, Worker,
// firmware), and its "Golden vectors" section pins these bytes.
//
// This file is PURE: X25519 comes from vendored Monocypher and HKDF-SHA256 /
// AES-GCM from mbedTLS, both of which build on the host as well as on device.
// test_relay_crypto.c therefore checks the REAL implementation against the
// contract's vectors rather than a stand-in -- which matters, because a
// derivation that is merely plausible produces a silently wrong key and then
// garbage frames, with nothing to point at.
//
