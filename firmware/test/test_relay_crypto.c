// test_relay_crypto.c — host tests for the cloud-relay crypto/wire layer.
...
static void test_status_body(void)
{
    ...
    /* Repeat of the same wake reproduces the body byte-for-byte. */
    char again[512];
    ...
    /* Empty name = no press, never a bare id. */
    CHECK(relay_build_status_body(b, sizeof b, "dev1", "1.10.0", 800, 480,
                                  NULL, 0, 0, 0, "", 7));
    CHECK(strstr(b, "\"button\"") == NULL);
    CHECK(strstr(b, "\"button_event_id\"") == NULL);
    /* Unknown optionals omitted, not zero-filled. */
    CHECK(strstr(b, "\"ip\"") == NULL);
    CHECK(strstr(b, "\"rssi\"") == NULL);
    CHECK(strstr(b, "\"battery_mv\"") == NULL);

    /* Large ids still print as an exact integer (cJSON %g goes scientific
     * above ~2^53, which would silently break the server's id dedup). */
